<script lang="ts">
	...
	function handleClickOutside(event: MouseEvent) {
		if (focused && container && list && !container.contains(event.target as Node)) {
			handleBlur();
		}
	}
	...
	$: selectedLabel =
		filterText === '' ? items.find((item) => item.value === value)?.label : filterText;
	...
	$: filteredItems = filterText == '' ? items : searcher.search(filterText).slice(0, 10);
...
<div class="relative" bind:this={container}>
	<div
		...
		role="listbox"
	>
		<span aria-live="polite" aria-atomic="false" aria-relevant="additions text" class="sr-only">
			{#if focused}
	...
	{#if listOpen}
		...
		>
			{#if filteredItems.length > 0}
				{#each filteredItems as item, i}
					<div
						...
						class:bg-surface-300={hoverItemIndex === i && !isItemActive(item, value)}
						class:dark:bg-surface-500={hoverItemIndex === i && !isItemActive(item, value)}
					>
			...
			{:else if !hideEmptyState}
				<slot name="empty">
					<div class="py-2 text-center text-gray-600 dark:text-gray-400">No options</div>
				</slot>
			{/if}
		</div>
	{/if}
